...
#if TEST_STD_VER >= 14
  mstd::map<std::string, int, std::less<>> strMap;
  const mstd::map<std::string, int, std::less<>> cstrMap{};
...
#if TEST_STD_VER >= 14
  mstd::map<int, int, TransparentCompare> tm;
  const mstd::map<int, int, TransparentCompare> ctm{};
