...
class set {
  ...
  public:
    ...
    template <class Key2,
              OrdersAtLeastWeakly<Key2> CompareT2,
    ...
    set() noexcept(std::is_nothrow_default_constructible_v<allocator_type>
    ...
    iterator insert(const_iterator pos, TransparentKey&& val) {
        return tree_.emplaceHintUnique(pos, std::forward<TransparentKey>(val))
            .position;
    }
    ...
    iterator insert(const_iterator hint, node_type&& nh) {
        MSTD_ASSERT_COMPATIBLE_ALLOCATOR(
            nh.empty() || (nh.get_allocator() == get_allocator()),
            "node_type with incompatible allocator passed to set::insert()");
        return tree_.template nodeHandleInsertUnique<node_type>(hint,
                                                                std::move(nh));
    }
    ...
    [[nodiscard]] auto lower_bound(this Self& self, const TransparentKey& k) {
        if constexpr (std::same_as<std::remove_cvref_t<TransparentKey>,
                                   key_type>) {
...
template <std::input_iterator                               IteratorT,
          OrdersAtLeastWeakly<std::iter_value_t<IteratorT>> CompareT
          = compare_three_way,
...
template <std::ranges::input_range                                RangeT,
          OrdersAtLeastWeakly<std::ranges::range_value_t<RangeT>> CompareT
          = compare_three_way,
          Allocator AllocatorT
          = std::allocator<std::ranges::range_value_t<RangeT>>>
...
template <class KeyT,
          OrdersAtLeastWeakly<KeyT> CompareT   = compare_three_way,
          Allocator                 AllocatorT = std::allocator<KeyT>>
set(std::initializer_list<KeyT>,
    CompareT   = CompareT(),
    AllocatorT = AllocatorT()) -> set<KeyT, CompareT, AllocatorT>;
...
set(std::initializer_list<KeyT>, AllocatorT)
    -> set<KeyT, compare_three_way, AllocatorT>;

template <class KeyT, class CompareT, class AllocatorT>
inline bool operator==(const set<KeyT, CompareT, AllocatorT>& lhs,
                       const set<KeyT, CompareT, AllocatorT>& rhs) {
    return lhs.size() == rhs.size()
           && std::equal(lhs.begin(), lhs.end(), rhs.begin());
}

template <class KeyT, class CompareT, class AllocatorT>
auto operator<=>(const set<KeyT, CompareT, AllocatorT>& lhs,
                 const set<KeyT, CompareT, AllocatorT>& rhs) {
    return std::lexicographical_compare_three_way(
        lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), CompareT{});
}

template <class KeyT, class CompareT, class AllocatorT>
inline void swap(
    set<KeyT, CompareT, AllocatorT>& lhs,
    set<KeyT, CompareT, AllocatorT>& rhs) noexcept(noexcept(lhs.swap(rhs))) {
    lhs.swap(rhs);
}
...
template <class KeyT, OrdersAtLeastWeakly<KeyT> CompareT, Allocator AllocatorT>
class multiset {
  public:
    // types:
    using key_type        = KeyT;
  ...
  public:
    ...
    multiset() noexcept(std::is_nothrow_default_constructible_v<allocator_type>
                        && std::is_nothrow_default_constructible_v<key_compare>
                        && std::is_nothrow_copy_constructible_v<key_compare>)
    : tree_(key_compare()) {
    }
    ...
    template <std::convertible_to<value_type> TransparentKey>
        requires OrdersWithAtLeastWeakly<key_compare,
                                         TransparentKey,
                                         value_type>
    ...
    iterator insert(const_iterator hint, node_type&& nh) {
        MSTD_ASSERT_COMPATIBLE_ALLOCATOR(
            ...
            "node_type with incompatible allocator passed to "
            "multiset::insert()");
        return tree_.template nodeHandleInsertMulti<node_type>(hint,
                                                               std::move(nh));
    }
...
template <class KeyT,
          OrdersAtLeastWeakly<KeyT> CompareT   = compare_three_way,
          Allocator                 AllocatorT = std::allocator<KeyT>>
multiset(std::initializer_list<KeyT>,
         CompareT   = CompareT(),
         AllocatorT = AllocatorT()) -> multiset<KeyT, CompareT, AllocatorT>;

template <std::input_iterator          IteratorT,
          std::sentinel_for<IteratorT> SentinelT,
          Allocator                    AllocatorT>
multiset(IteratorT, SentinelT, AllocatorT)
    -> multiset<std::iter_value_t<IteratorT>, compare_three_way, AllocatorT>;

template <std::ranges::input_range RangeT, Allocator AllocatorT>
multiset(RangeT&&, AllocatorT) -> multiset<std::ranges::range_value_t<RangeT>,
                                           compare_three_way,
                                           AllocatorT>;
...
template <class KeyT, class CompareT, class AllocatorT>
inline bool operator==(const multiset<KeyT, CompareT, AllocatorT>& lhs,
                       const multiset<KeyT, CompareT, AllocatorT>& rhs) {
    return lhs.size() == rhs.size()
           && std::equal(lhs.begin(), lhs.end(), rhs.begin());
}

template <class KeyT, class CompareT, class AllocatorT>
auto operator<=>(const multiset<KeyT, CompareT, AllocatorT>& lhs,
                 const multiset<KeyT, CompareT, AllocatorT>& rhs) {
    return std::lexicographical_compare_three_way(
        lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), CompareT{});
}
...
template <class KeyT, class CompareT, class AllocatorT>
inline void swap(multiset<KeyT, CompareT, AllocatorT>& lhs,
...
namespace pmr {
template <class KeyT, class CompareT = compare_three_way>
using set = mstd::set<KeyT, CompareT, std::pmr::polymorphic_allocator<KeyT>>;

template <class KeyT, class CompareT = compare_three_way>
using multiset
    = mstd::multiset<KeyT, CompareT, std::pmr::polymorphic_allocator<KeyT>>;
} // namespace pmr
