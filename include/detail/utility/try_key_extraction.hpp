...
concept reference_to
    = std::same_as<KeyT, std::remove_const_t<std::remove_reference_t<T>>>;
...
decltype(auto) try_key_extraction_with_key(WithKeyT             with_key,
                                           MapValue<KeyT, T2>&& arg) {
    return with_key(arg.key(), std::move(arg));
}

template <class KeyT, class WithKeyT, class T2>
decltype(auto) try_key_extraction_with_key(WithKeyT                  with_key,
                                           const MapValue<KeyT, T2>& arg) {
    return with_key(arg.key(), arg);
}

template <class KeyT, class WithKeyT, reference_to<KeyT> ArgT1, class ArgT2>
decltype(auto)
    try_key_extraction_with_key(WithKeyT with_key, ArgT1&& arg1, ArgT2&& arg2) {
    return with_key(arg1, std::forward<ArgT1>(arg1), std::forward<ArgT2>(arg2));
}

template <class KeyT,
          class WithKeyT,
          reference_to<std::piecewise_construct_t> PiecewiseConstructT,
          reference_to<KeyT>                       T1,
          class TupleT2>
decltype(auto) try_key_extraction_with_key(WithKeyT              with_key,
                                           PiecewiseConstructT&& pc,
                                           std::tuple<T1>&&      tuple1,
                                           TupleT2&&             tuple2) {
    return with_key(std::get<0>(tuple1),
...
// If it succeeds to extract the key, it calls the `with_key` function with the
// extracted key and all of the arguments. Otherwise it calls the `without_key`
// function with all of the arguments.
//
...
decltype(auto) try_key_extraction(WithKeyT    with_key,
                                  WithoutKeyT without_key,
                                  ArgsT&&... args) {
    if constexpr (requires {
                      try_key_extraction_with_key<KeyT>(
                          with_key, std::forward<ArgsT>(args)...);
                  }) {
        return try_key_extraction_with_key<KeyT>(with_key,
