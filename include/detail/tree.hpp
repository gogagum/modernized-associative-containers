...
// GCC complains about the backslashes at the end, see
// https://gcc.gnu.org/bugzilla/show_bug.cgi?id=121528 Tree is a red-black-tree
// implementation used for the associative containers (i.e. (multi)map/set). It
// stores
// - (1) a pointer to the node with the smallest (i.e. leftmost) element, namely
// begin_node_
// - (2) the number of nodes in the tree, namely size_
// - (3) a pointer to the root of the tree, namely end_node_
//
// Storing (1) and (2) is required to allow for constant time lookups. A tree
...
//   ...  ...  ...   ...
//
// All nodes except end_node_ have a left_ and right_ pointer as well as a
// parent_ pointer. end_node_ only contains a left_ pointer, which points to the
// root of the tree. This layout allows for iteration through the tree without a
// need for special handling of the end node. See tree_next_iter and
// tree_prev_iter for more details.
...
class Tree {
  ...
  private:
    ...
    using KeyType_     = std::remove_cvref_t<decltype(std::invoke(
        std::declval<const key_proj&>(), std::declval<value_type&>()))>;
    static_assert(OrdersAtLeastWeakly<key_compare, KeyType_>,
                  "key_compare must be at least a weak ordering on the "
                  "projected key type");
  ...
  private:
    ...
    class NodeBase : public EndNode {
      public:
        ...
        pointer parent_unsafe() const {
            return static_cast<pointer>(parent_);
        }
    ...
    class Node : public NodeBase {
      public:
        // We use a union to avoid initialization during member initialization,
        // which allows us to use the allocator from the container to construct
        // the `node_value_type` in the memory provided by the union member
          ...
          public:
            ...
            void operator()(pointer ptr) noexcept {
                if (value_constructed) {
                    AllocTraits_::destroy(na_,
                                          std::addressof(ptr->get_value()));
                }
                if (ptr) {
      ...
      public:
        ...
        explicit Node(AllocT& node_alloc, ArgsT&&... args) {
            std::allocator_traits<AllocT>::construct(
                node_alloc,
                std::addressof(get_value()),
  ...
  public:
    ...
    using const_end_node_pointer
        = void_pointer_traits::template rebind<const end_node_t>;
    using node_allocator = AllocTraits_::template rebind_alloc<node>;
    using node_traits    = std::allocator_traits<node_allocator>;

  private:
    template <class Self>
    using SelfEndNodePointer = std::conditional_t<std::is_const_v<Self>,
                                                  const_end_node_pointer,
                                                  end_node_pointer>;

    template <class Self>
    using SelfNodeBasePointerRef = std::conditional_t<std::is_const_v<Self>,
                                                      const node_base_pointer,
                                                      node_base_pointer>&;
    ...
    static_assert(
        std::is_same_v<node_base_pointer, typename NodeBaseTraits_::pointer>,
        "Allocator does not rebind pointers in a sane manner.");
    ...
    class TreeIterator {
      ...
      public:
        ...
        TreeIterator(TreeIterator<other_is_const> non_const_iter) noexcept
            requires(is_const || (other_is_const == is_const))
        ...
        TreeIterator& operator++() {
            ptr_ = mstd::detail::tree_next_iter<end_node_pointer>(
                static_cast<node_base_pointer>(ptr_));
            return *this;
        }
        ...
        TreeIterator& operator--() {
            ptr_ = static_cast<end_node_pointer>(
                mstd::detail::tree_prev_iter<node_base_pointer>(ptr_));
            return *this;
        }
      ...
      private:
        explicit TreeIterator(node_pointer ptr) noexcept
        : ptr_(ptr) {
        }
        explicit TreeIterator(end_node_pointer ptr) noexcept
        : ptr_(ptr) {
        }
        // TODO(gogagum): const_cast should be removed if end method will start
        // to return some sentinel iterator.
        explicit TreeIterator(const_end_node_pointer ptr) noexcept
            requires is_const
        : ptr_(const_cast<end_node_pointer>(ptr)) {
        }
  ...
  public:
    ...
    auto endNode(this Self& self) noexcept {
        return std::pointer_traits<SelfEndNodePointer<Self>>::pointer_to(
            self.end_node_);
    }

    node_allocator& nodeAlloc() noexcept {
        return node_alloc_;
    }

  private:
    const node_allocator& nodeAlloc() const noexcept {
        return node_alloc_;
    }

  public:
    allocator_type alloc() const noexcept {
        return allocator_type(nodeAlloc());
    }
  ...
  public:
    ...
    : begin_node_(endNode())
    , node_alloc_(
          node_traits::select_on_container_copy_construction(other.nodeAlloc()))
    , size_(0)
    , key_comp_(other.key_comp()) {
        ...
        *root_ptr()
            = static_cast<node_base_pointer>(copyConstructTree_(other.root()));
        root()->parent_ = endNode();
        begin_node_     = static_cast<end_node_pointer>(
            mstd::detail::tree_min(endNode()->left_));
        size_ = other.size();
    }
    ...
    Tree& operator=(const Tree& other) {
        ...
        if (size_ != 0) {
            *root_ptr() = static_cast<node_base_pointer>(
                copyAssignTree(root(), other.root()));
        } else {
            *root_ptr() = static_cast<node_base_pointer>(
                copyConstructTree_(other.root()));
            if (root()) {
                root()->parent_ = endNode();
            }
        }
        begin_node_ = endNode()->left_
                          ? static_cast<end_node_pointer>(
                                mstd::detail::tree_min(endNode()->left_))
                          : endNode();
        size_       = other.size();
    ...
    , key_comp_(std::move(other.key_comp_)) {
        if (size_ == 0) {
            ...
            endNode()->left_->parent_
                = static_cast<end_node_pointer>(endNode());
    ...
    , key_comp_(std::move(other.key_comp())) {
        ...
        if (alloc == other.alloc()) {
            ...
            *root_ptr() = static_cast<node_base_pointer>(
                moveConstructTree_(other.root()));
            root()->parent_ = endNode();
            begin_node_     = static_cast<end_node_pointer>(
                mstd::detail::tree_min(endNode()->left_));
            size_ = other.size();
            other.clear(); // Ensure that other is in a valid state after moving
                           // out the keys
        }
    }
    ...
    ~Tree() {
        static_assert(std::is_copy_constructible<key_compare>::value,
    ...
    size_type max_size() const noexcept {
        return std::min<size_type>(node_traits::max_size(nodeAlloc()),
    ...
    iterator emplaceMulti(ArgsT&&... args) {
        ...
        insertNodeAt(
            parent, child, static_cast<node_base_pointer>(holder.get()));
    ...
    insert_return_type emplaceUnique(ArgsT&&... args) {
        return mstd::try_key_extraction<KeyType_>(
            [this](const KeyType_& key, ArgsT&&... args2) {
                ...
                if (child == nullptr) {
                    node_holder holder
                        = constructNode_(std::forward<ArgsT>(args2)...);
                    insertNodeAt(parent,
                                 child,
    ...
    void insertRangeMulti(InIterT begin, SentT end) {
        ...
        if (root() == nullptr) { // Make sure we always have a root node
            insertNodeAt(endNode(),
                         endNode()->left_,
                         static_cast<node_base_pointer>(
                             constructNode_(*begin).release()));
            ++begin;
        }

        auto max_node = static_cast<node_pointer>(
            mstd::detail::tree_max(static_cast<node_base_pointer>(root())));

        for (; begin != end; ++begin) {
            auto        holder       = constructNode_(*begin);
            const auto& max_node_key = key_proj_(max_node->get_value());
            const auto& holder_key   = key_proj_(holder->get_value());
            // Always check the max node first. This optimizes for sorted ranges
            // inserted at the end.
            if (key_comp_(max_node_key, holder_key) >= 0) { // node >= __max_val
                ...
                node_base_pointer& child
                    = findLeafHigh_(parent, holder->get_value());
                insertNodeAt(parent,
                             child,
                             static_cast<node_base_pointer>(holder.release()));
            }
        }
    }
            ...
            mstd::try_key_extraction<KeyType_>(
                [this, &max_node](const KeyType_& key, Reference&& val) {
                    ...
                    if (key_comp_(max_node_key, key) < 0) { // key > max_node
                        auto holder
                            = constructNode_(std::forward<Reference>(val));
                        insertNodeAt(
                            static_cast<end_node_pointer>(max_node),
                            max_node->right_,
                            static_cast<node_base_pointer>(holder.get()));
                        max_node = holder.release();
                    } else {
                ...
                [this, &max_node](Reference&& val) {
                    ...
                    const auto& max_node_key = key_proj_(max_node->get_value());
                    const auto& holder_key   = key_proj_(holder->get_value());
                    if (key_comp_(max_node_key, holder_key)
                        < 0) { // node > max_node
                        insertNodeAt(
                            static_cast<end_node_pointer>(max_node),
                            max_node->right_,
                        ...
                        auto [parent, child]
                            = find_equivalent(key_proj_(holder->get_value()));
                        if (child == nullptr) {
                            insertNodeAt(parent,
                                         child,
                                         static_cast<node_base_pointer>(
    ...
    iterator removeNodePointer(node_pointer ptr) noexcept {
        ...
        --size_;
        mstd::detail::tree_remove(endNode()->left_,
                                  static_cast<node_base_pointer>(ptr));
        return ret;
    }
    ...
    void nodeHandleMergeUnique(
        Tree<ValueT, KeyProj, Comp2T, AllocatorT>& source) {
        for (iterator iter = source.begin(); iter != source.end();) {
            ...
            insertNodeAt(
                parent, child, static_cast<node_base_pointer>(src_ptr));
        }
    }
    ...
    iterator erase(const_iterator pos) {
        ...
        node_traits::destroy(node_allocator,
                             std::addressof(node_ptr->get_value()));
    ...
    void insertNodeAt(end_node_pointer   parent,
                      node_base_pointer& child,
                      node_base_pointer  new_node) noexcept {
    ...
    size_type countMulti(const KeyT& key) const {
        ...
        while (root_node != nullptr) {
            ...
            if (comp_res < 0) {
                ...
                return std::distance(
                    const_iterator{lowerUpperBoundMultiImpl_<true>(
                        key,
                        static_cast<node_pointer>(root_node->left_),
                        static_cast<end_node_pointer>(root_node))},
                    const_iterator{lowerUpperBoundMultiImpl_<false>(
                        key,
    ...
    auto lowerBoundUnique(this Self& self, const KeyT& key) {
        return SelfIterator<Self>{
            self.template lowerUpperBoundUniqueImpl_<true>(key)};
    }

    template <class Self, class KeyT>
    auto upperBoundUnique(this Self& self, const KeyT& key) {
        return SelfIterator<Self>(
            self.template lowerUpperBoundUniqueImpl_<false>(key));
    }
  ...
  private:
        ...
        while (root_node != nullptr) {
            ...
            if (comp_res < 0) {
                ...
                return root_node->right_
                           ? static_cast<end_node_pointer>(
                                 mstd::detail::tree_min(root_node->right_))
                           : result;
            }
        }
        return result;
    ...
    auto lowerUpperBoundMultiImpl_(this Self&               self,
                                   const KeyT&              key,
                                   node_pointer             root_node,
                                   SelfEndNodePointer<Self> result) {
        while (root_node != nullptr) {
  ...
  public:
    ...
    auto lowerBoundMulti(this Self& self, const KeyT& key) {
        return SelfIterator<Self>{self.template lowerUpperBoundMultiImpl_<true>(
            key, self.root(), self.endNode())};
    }

    template <class Self, class KeyT>
    auto upperBoundMulti(this Self& self, const KeyT& key) {
        return SelfIterator<Self>{
            self.template lowerUpperBoundMultiImpl_<false>(
                key, self.root(), self.endNode())};
    }
  ...
  public:
        ...
        while (root_node != nullptr) {
            ...
            if (comp_res < 0) {
                ...
                auto end = self.template lowerUpperBoundMultiImpl_<false>(
                    key, static_cast<node_pointer>(root_node->right_), result);
                return SelfSubrange<Self>{
    ...
    using node_holder
        = std::unique_ptr<node,
                          typename node::template Destructor<node_allocator>>;
    ...
    node_holder remove(const_iterator pos) noexcept {
        ...
        --size_;
        mstd::detail::tree_remove(endNode()->left_,
                                  static_cast<node_base_pointer>(node_ptr));
        return node_holder{
            node_ptr,
    ...
    // Find key
    // If key exists, return the parent of the node of key and a reference to
    // the pointer to the node of key. If key doesn't exist, return the parent
    // of the null leaf and a reference to the pointer to the null leaf.
    template <class Self, class KeyT>
        ...
        requires OrdersWithAtLeastWeakly<key_compare, KeyType_, KeyT>
    {
        ...
        while (true) {
            const std::weak_ordering comp_res
                = self.key_comp_(key, self.key_proj_(node_ptr->get_value()));
    ...
    // of the null leaf and a reference to the pointer to the null leaf.
    template <class KeyT>
    std::pair<end_node_pointer, node_base_pointer&> find_equivalent(
        const_iterator hint, node_base_pointer& dummy, const KeyT& key)
        requires OrdersWithAtLeastWeakly<key_compare, KeyType_, KeyT>
    {
        const auto& hint_key = key_proj_(*hint);
        if (hint == end()
            || std::is_lt(key_comp_(key, hint_key))) { // check before
            // key < *hint
            const_iterator prior = hint;
            if (prior == begin()
                || (std::is_gt(key_comp_(key, key_proj_(*--prior))))) {
                // *prev(hint) < key < *hint
        ...
        if (std::is_gt(key_comp_(key, hint_key))) { // check after
            ...
            if (next == end()
                || (std::is_lt(key_comp_(key, key_proj_(*next))))) {
  ...
  private:
    ...
    node_base_pointer& findLeafLow_(end_node_pointer& parent,
    ...
    // Set parent to parent of null leaf
    // Return reference to null leaf
    node_base_pointer& findLeafHigh_(end_node_pointer& parent,
                                     const value_type& value) {
        auto node_ptr = root();
        if (node_ptr != nullptr) {
            while (true) {
    ...
    // Set parent to parent of null leaf
    // Return reference to null leaf
    node_base_pointer& findLeaf_(const_iterator    hint,
                                 end_node_pointer& parent,
                                 const value_type& value) {
        auto& value_key = key_proj_(value);
        auto& hint_key  = key_proj_(*hint);
        if (hint == end()
            || (key_comp_(hint_key, value_key) >= 0)) // check before
        {
            // value <= *hint
            const_iterator prior = hint;
            if (prior == begin()
                || (key_comp_(value_key, key_proj_(*--prior)) >= 0)) {
    ...
    node_holder constructNode_(ArgsT&&... args) {
        ...
        std::construct_at(
            std::addressof(*holder), node_alloc, std::forward<ArgsT>(args)...);
    ...
    void moveAssign_(Tree& other) {
        ...
        if (size_ != 0) {
            *root_ptr() = static_cast<node_base_pointer>(
                moveAssignTree(root(), other.root()));
        } else {
            *root_ptr() = static_cast<node_base_pointer>(
                moveConstructTree_(other.root()));
            if (root()) {
        ...
        begin_node_ = endNode()->left_
                          ...
                          : endNode();
        size_       = other.size();
        other.clear(); // Ensure that other is in a valid state after moving out
                       // the keys
    }
    ...
    class TreeDeleter {
      ...
      public:
        TreeDeleter(node_allocator& alloc)
        : alloc_(alloc) {
        }

        void operator()(node_pointer node_ptr) {
            if (!node_ptr) {
                return;
            }
    ...
    // This copy construction will always produce a correct red-black-tree
    // assuming the incoming tree is correct, since we copy the exact structure
    // 1:1. Since this is for copy construction _only_ we know that we get a
    // correct tree. If we didn't get a correct tree, the invariants of Tree are
    // broken and we have a much bigger problem than an improperly balanced
    // tree.
    template <class NodeConstructorT>
    node_pointer constructFromTree_(node_pointer     src,
                                    NodeConstructorT construct) {
        if (!src) {
            return nullptr;
        }
        ...
        std::unique_ptr<node, TreeDeleter> left{
            constructFromTree_(static_cast<node_pointer>(src->left_),
                               construct),
            node_alloc_,
        };
        node_pointer right = constructFromTree_(
            static_cast<node_pointer>(src->right_), construct);
        ...
        if (new_node_ptr->left_) {
            new_node_ptr->left_->parent_
                = static_cast<end_node_pointer>(new_node_ptr);
        }
        if (new_node_ptr->right_) {
            new_node_ptr->right_->parent_
                = static_cast<end_node_pointer>(new_node_ptr);
        }
        return new_node_ptr;
    }
    ...
    node_pointer copyConstructTree_(node_pointer src) {
        return constructFromTree_(
            src, [this](const value_type& val) { return constructNode_(val); });
    }
    ...
    // This copy assignment will always produce a correct red-black-tree
    // assuming the incoming tree is correct, since our own tree is a
    // red-black-tree and the incoming tree is a red-black-tree. The invariants
    // of a red-black-tree are temporarily not met until all of the incoming
    // red-black tree is copied.
    node_pointer assignFromTree(node_pointer     dest,
                                node_pointer     src,
                                AssignmentT      assign,
                                ConstructionAlgT construct_subtree) {
        if (!src) {
            destroy_(dest);
            return nullptr;
        }
        ...
        // If we already have a left node in the destination tree, reuse it and
        // copy-assign recursively
        if (dest->left_) {
            dest->left_ = static_cast<node_base_pointer>(
                assignFromTree(static_cast<node_pointer>(dest->left_),
            ...
            auto new_left
                = construct_subtree(static_cast<node_pointer>(src->left_));
        ...
        if (dest->right_) {
            dest->right_ = static_cast<node_base_pointer>(
                assignFromTree(static_cast<node_pointer>(dest->right_),
                               static_cast<node_pointer>(src->right_),
                               assign,
                               construct_subtree));
        } else if (src->right_) {
            auto new_right
                = construct_subtree(static_cast<node_pointer>(src->right_));
    ...
    node_pointer copyAssignTree(node_pointer dest, node_pointer src) {
        return assignFromTree(
            dest,
            src,
            [](value_type& lhs, const value_type& rhs) {
                assignValue_(lhs, rhs);
            },
            [this](node_pointer node_ptr) {
                return copyConstructTree_(node_ptr);
            });
    }
    ...
    node_pointer moveAssignTree(node_pointer dest, node_pointer src) {
        return assignFromTree(
            ...
            [](value_type& lhs, value_type& rhs) {
                assignValue_(lhs, std::move(rhs));
            },
            [this](node_pointer node_ptr) {
                return moveConstructTree_(node_ptr);
            });
    }
};
