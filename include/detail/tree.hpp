...
public:
    __end_node_pointer endNode() noexcept {
        return std::pointer_traits<__end_node_pointer>::pointer_to(__end_node_);
    }
    __end_node_pointer endNode() const noexcept {
        return std::pointer_traits<__end_node_pointer>::pointer_to(const_cast<__end_node_t&>(__end_node_));
    }
    __node_allocator& nodeAlloc() noexcept { return __node_alloc_; }

private:
    const __node_allocator& nodeAlloc() const noexcept { return __node_alloc_; }
...
public:
    ...
    , __value_comp_(other.value_comp()) {
        ...
        *root_ptr()       = static_cast<__node_base_pointer>(copy_construct_tree(other.root()));
        root()->__parent_ = endNode();
        __begin_node_       = static_cast<__end_node_pointer>(mstd::tree_min(endNode()->__left_));
        __size_             = other.size();
    }
    ...
    noexcept(
     ...
     && (
        ...
        moveAssign(other, std::integral_constant<bool, __node_traits::propagate_on_container_move_assignment::value>());
        return *this;
    }
    ...
    size_type max_size() const noexcept {
        return std::min<size_type>(
            __node_traits::max_size(nodeAlloc()),
    ...
    iterator emplaceMulti(ArgsT&&... args) {
        __node_holder __h = construct_node(std::forward<ArgsT>(args)...);
    ...
    std::pair<iterator, bool> emplaceUnique(ArgsT&&... args) {
        return mstd::__try_key_extraction<key_type>(
            [this](const key_type& key, ArgsT&&... args2) {
                ...
                bool inserted        = false;
                if (child == nullptr) {
                    __node_holder holder = construct_node(std::forward<ArgsT>(args2)...);
    ...
    std::pair<iterator, bool> emplaceHintUnique(const_iterator pos, ArgsT&&... args) {
        return mstd::__try_key_extraction<key_type>(
            [this, pos](const key_type& key, ArgsT&&... args2) {
                ...
                bool inserted        = false;
                if (child == nullptr) {
                    __node_holder __h = construct_node(std::forward<ArgsT>(args2)...);
                    insertNodeAt(parent, child, static_cast<__node_base_pointer>(__h.get()));
                    ret        = __h.release();
                    inserted = true;
                }
            ...
            [this, pos](ArgsT&&... args2) {
                __node_holder __h = construct_node(std::forward<ArgsT>(args2)...);
                __node_base_pointer dummy;
                auto [parent, child] = find_equal(pos, dummy, __h->get_value());
                auto ret             = static_cast<__node_pointer>(child);
                if (child == nullptr) {
                    insertNodeAt(parent, child, static_cast<__node_base_pointer>(__h.get()));
                    ret = __h.release();
    ...
    void __insert_range_multi(_InIter begin, _Sent end) {
        ...
        if (root() == nullptr) { // Make sure we always have a root node
            insertNodeAt(
                endNode(),
                endNode()->__left_,
                static_cast<__node_base_pointer>(construct_node(*begin).release())
            );
            ++begin;
        }
        ...
        for (; begin != end; ++begin) {
            __node_holder __nd = construct_node(*begin);
            // Always check the max node first. This optimizes for sorted ranges inserted at the end.
            if (!value_comp()(__nd->get_value(), max_node->get_value())) { // __node >= __max_val
                insertNodeAt(static_cast<__end_node_pointer>(max_node),
                             max_node->__right_,
                ...
                insertNodeAt(parent, child, static_cast<__node_base_pointer>(__nd.release()));
            }
        }
    }
    ...
    void __insert_range_unique(_InIter __first, _Sent __last) {
        ...
        if (root() == nullptr) {
            insertNodeAt(
                endNode(), endNode()->__left_, static_cast<__node_base_pointer>(construct_node(*__first).release()));
            ++__first;
        }
            ...
            mstd::__try_key_extraction<key_type>(
                [this, &__max_node](const key_type& __key, __reference&& __val) {
                    if (value_comp()(__max_node->get_value(), __key)) { // __key > __max_node
                        __node_holder __nd = construct_node(std::forward<__reference>(__val));
                        insertNodeAt(static_cast<__end_node_pointer>(__max_node),
                                         __max_node->__right_,
                                         static_cast<__node_base_pointer>(__nd.get()));
                        __max_node = __nd.release();
                    } else {
                        auto [parent, child] = find_equal(__key);
                        if (child == nullptr) {
                            __node_holder __nd = construct_node(std::forward<__reference>(__val));
                            insertNodeAt(parent, child, static_cast<__node_base_pointer>(__nd.release()));
                        }
    ...
    void insertNodeAt(__end_node_pointer parent, __node_base_pointer& child, __node_base_pointer new_node) noexcept {
...
private:
    template <class KeyT>
    iterator
    __lower_bound_multi(const KeyT& key, __node_pointer root, __end_node_pointer result);
...
public:
    ...
    iterator __lower_bound_multi(const KeyT& __v) {
        return __lower_bound_multi(__v, root(), endNode());
    }
    template <class KeyT>
    ...
    iterator __upper_bound_multi(const KeyT& __v) {
        return __upper_bound_multi(__v, root(), endNode());
    }
...
public:
    ...
    __node_holder remove(const_iterator pos) noexcept;

    // FIXME: Make this function const qualified. Unfortunately doing so
    // breaks existing code which uses non-const callable comparators.
    template <class KeyT>
    std::pair<__end_node_pointer, __node_base_pointer&> find_equal(const KeyT& __v);

    template <class KeyT>
    std::pair<__end_node_pointer, __node_base_pointer&> find_equal(const KeyT& __v) const {
        return const_cast<Tree*>(this)->find_equal(__v);
    }

    template <class KeyT>
    std::pair<__end_node_pointer, __node_base_pointer&>
    find_equal(const_iterator hint, __node_base_pointer& dummy, const KeyT& __v);

    void copyAssignAlloc(const Tree& other) {
        copyAssignAlloc(other, std::integral_constant<bool, __node_traits::propagate_on_container_copy_assignment::value>());
    }
...
private:
    __node_base_pointer& __find_leaf_low(__end_node_pointer& parent, const value_type& __v);

    __node_base_pointer& __find_leaf_high(__end_node_pointer& parent, const value_type& __v);

    __node_base_pointer&
    __find_leaf(const_iterator hint, __end_node_pointer& parent, const value_type& __v);

    template <class... ArgsT>
    __node_holder construct_node(ArgsT&&... args);

    void destroy(__node_pointer __nd) noexcept { (TreeDeleter(__node_alloc_))(__nd); }
    ...
    void __move_assign_alloc(Tree& other) noexcept(
     ...
     || std::is_nothrow_move_assignable<__node_allocator>::value
        ...
        __move_assign_alloc(other, std::integral_constant<bool, __node_traits::propagate_on_container_move_assignment::value>());
    }
    ...
    __node_pointer __construct_from_tree(__node_pointer __src, _NodeConstructor __construct) {
        ...
        __new_node_ptr->__right_    = static_cast<__node_base_pointer>(right);
    ...
    __node_pointer copy_construct_tree(__node_pointer src) {
        return __construct_from_tree(src, [this](const value_type& val) { return construct_node(val); });
    }
    ...
    __node_pointer move_construct_tree(__node_pointer src) {
        return __construct_from_tree(src, [this](value_type& val) {
            return construct_node(const_cast<key_type&&>(val.first), std::move(val.second));
        });
    }
    ...
    __node_pointer move_construct_tree(__node_pointer src) {
        return __construct_from_tree(src, [this](value_type& val) {
            return construct_node(std::move(val));
        });
    }
...
Tree<_Tp, _Compare, _Allocator>& Tree<_Tp, _Compare, _Allocator>::operator=(const Tree& other) {
    ...
    if (__size_ != 0) {
        *root_ptr() = static_cast<__node_base_pointer>(copyAssignTree(root(), other.root()));
    } else {
        *root_ptr() = static_cast<__node_base_pointer>(copy_construct_tree(other.root()));
        if (root()) {
            root()->__parent_ = endNode();
    ...
    __begin_node_
        = endNode()->__left_
        ? static_cast<__end_node_pointer>(mstd::tree_min(endNode()->__left_))
        : endNode();
    __size_ = other.size();
...
: __begin_node_(endNode())
, __node_alloc_(__node_traits::select_on_container_copy_construction(other.nodeAlloc()))
, __size_(0)
, __value_comp_(other.value_comp()) {
    if (other.size() == 0)
        return;
...
, __value_comp_(std::move(other.__value_comp_)) {
    if (__size_ == 0) {
        ...
        endNode()->__left_->__parent_ = static_cast<__end_node_pointer>(endNode());
...
, __value_comp_(std::move(other.value_comp())) {
    ...
    if (alloc == other.alloc()) {
        ...
        *root_ptr()     = static_cast<__node_base_pointer>(move_construct_tree(other.root()));
        root()->__parent_ = endNode();
        __begin_node_     = static_cast<__end_node_pointer>(mstd::tree_min(endNode()->__left_));
        __size_           = other.size();
...
noexcept(
 ...
 && std::is_nothrow_move_assignable<__node_allocator>::value
    ...
    destroy(static_cast<__node_pointer>(endNode()->__left_));
    __begin_node_ = other.__begin_node_;
...
void Tree<_Tp, _Compare, _Allocator>::moveAssign(Tree& other, std::false_type) {
    if (nodeAlloc() == other.nodeAlloc()) {
        ...
        if (__size_ != 0) {
            *root_ptr() = static_cast<__node_base_pointer>(moveAssignTree(root(), other.root()));
        } else {
            *root_ptr() = static_cast<__node_base_pointer>(move_construct_tree(other.root()));
            if (root())
                root()->__parent_ = endNode();
...
auto Tree<_Tp, _Compare, _Allocator>::__find_leaf(
    const_iterator hint, __end_node_pointer& parent, const value_type& __v)
  -> __node_base_pointer& {
    if (hint == end() || !value_comp()(*hint, __v)) // check before
    {
        ...
        if (__prior == begin() || !value_comp()(__v, *--__prior)) {
            ...
            if (hint.__ptr_->__left_ == nullptr) {
                parent = static_cast<__end_node_pointer>(hint.__ptr_);
                return parent->__left_;
            } else {
                parent = static_cast<__end_node_pointer>(__prior.__ptr_);
                return static_cast<__node_base_pointer>(__prior.__ptr_)->__right_;
            }
        }
        // __v < *prev(hint)
        return __find_leaf_high(parent, __v);
    }
    // else __v > *hint
...
template <class _Tp, class _Compare, class _Allocator>
template <class KeyT>
auto Tree<_Tp, _Compare, _Allocator>::find_equal(const_iterator hint, __node_base_pointer& dummy, const KeyT& key)
  -> std::pair<__end_node_pointer, __node_base_pointer&> {
    ...
    // else key == *hint
    dummy = static_cast<__node_base_pointer>(hint.__ptr_);
    return {
        hint.__ptr_,
        dummy,
    };
...
template <class _Tp, class _Compare, class _Allocator>
template <class... ArgsT>
auto Tree<_Tp, _Compare, _Allocator>::construct_node(ArgsT&&... args) -> __node_holder {
    __node_allocator& __na = nodeAlloc();
...
auto Tree<_Tp, _Compare, _Allocator>::__remove_node_pointer(__node_pointer ptr) noexcept -> iterator {
    ...
    --__size_;
    mstd::__tree_remove(endNode()->__left_, static_cast<__node_base_pointer>(ptr));
    return ret;
}
...
void Tree<_Tp, _Compare, _Allocator>::__node_handle_merge_unique(Tree<_Tp, _Comp2, _Allocator>& source) {
    for (iterator iter = source.begin(); iter != source.end();) {
        ...
        insertNodeAt(parent, child, static_cast<__node_base_pointer>(src_ptr));
    }
}
...
Tree<_Tp, _Compare, _Allocator>::remove(const_iterator pos) noexcept {
    ...
    if (__begin_node_ == pos.__ptr_) {
        if (node_ptr->__right_ != nullptr) {
            __begin_node_ = static_cast<__end_node_pointer>(node_ptr->__right_);
        } else {
            __begin_node_ = static_cast<__end_node_pointer>(node_ptr->__parent_);
        }
    }
    --__size_;
    mstd::__tree_remove(endNode()->__left_, static_cast<__node_base_pointer>(node_ptr));
    return __node_holder(node_ptr, _Dp(nodeAlloc(), true));
}
