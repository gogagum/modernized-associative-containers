...
class MapValue {
  public:
    ...
    MapValue(MapValue&&) noexcept(
        std::is_nothrow_move_constructible_v<KeyT>
        && std::is_nothrow_move_constructible_v<ValueT>) = default;
    // TODO(gogagum): think of a constructor that gets key and value via
    // structured binding
    ...
    MapValue(std::pair<KeyT, ValueT>&& kv_pair)
    : key_(std::move(kv_pair.first))
    , value_(std::move(kv_pair.second)) {
    }
    ...
    MapValue(std::tuple<KeyT, ValueT>&& kv_pair)
    : key_(std::move(std::get<0>(kv_pair)))
    , value_(std::move(std::get<1>(kv_pair))) {
    }
    ...
    MapValue(std::piecewise_construct_t,
             std::tuple<KeyArgsT...>   key_args,
             std::tuple<ValueArgsT...> value_args)
  ...
  private:
    ...
    MapValue& operator=(MapValue&&) noexcept(
        std::is_nothrow_move_assignable_v<KeyT>
        && std::is_nothrow_move_assignable_v<ValueT>) = default;
...
template <class KT, class VT>
MapValue(KT&& key, VT&& value)
    -> MapValue<std::remove_cvref_t<KT>, std::remove_cvref_t<VT>>;
