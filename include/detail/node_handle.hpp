...
class NodeHandle {
    ...
    using NodePointerType_
        = std::pointer_traits<VoidPointer_>::template rebind<NodeT>;
  ...
  private:
    ...
    void destroyNodePointer_() {
        if (ptr_ != nullptr) {
            ...
            typename NodeT::template Destructor<NodeAllocType>{alloc,
                                                               true}(ptr_);
            ptr_ = nullptr;
        }
    }
  ...
  public:
    ...
    void swap(NodeHandle& other) noexcept(
        ...
        if (AllocTraits_::propagate_on_container_swap::value
            || alloc_ == std::nullopt || other.alloc_ == std::nullopt) {
            std::swap(alloc_, other.alloc_);
        }
    }

    friend void swap(NodeHandle& lhs,
                     NodeHandle& rhs) noexcept(noexcept(lhs.swap(rhs))) {
        lhs.swap(rhs);
    }
