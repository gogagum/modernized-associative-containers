...
class map {
  ...
  private:
    using Tree_        = mstd::Tree<value_type,
                                    typename value_type::key_proj_,
                                    key_compare,
                                    allocator_type>;
  ...
  public:
    ...
    using node_handle_insert_return_type
        = NodeHandleInsertReturnType<iterator, node_type>;
    using insert_return_type = InsertReturnType<iterator>;

    template <class KeyT2,
              class /*Value*/,
              OrdersAtLeastWeakly<KeyT2> CompareT2,
              Allocator                  AllocatorT2>
    friend class map;
    template <class KeyT2,
              class /*Value*/,
              OrdersAtLeastWeakly<KeyT2> CompareT2,
    ...
    map() noexcept(std::is_nothrow_default_constructible_v<allocator_type>
    ...
    template <_ContainerCompatibleIterator<value_type> IteratorT,
    ...
    template <_ContainerCompatibleRange<value_type> RangeT>
    map(RangeT&& range, const allocator_type& alloc)
    : map(std::forward<RangeT>(range), key_compare(), alloc) {
    }
    ...
    mapped_type& operator[](const key_type& key) {
        return tree_
            .emplaceUnique(std::piecewise_construct,
    ...
    iterator emplace_hint(const_iterator pos, ArgsT&&... args) {
        return tree_.emplaceHintUnique(pos, std::forward<ArgsT>(args)...)
            .position;
    }
    ...
    iterator insert(const_iterator pos, P&& value) {
        return tree_.emplaceHintUnique(pos, std::forward<P>(value)).position;
    }
    ...
    void insert_range(RangeT&& range) {
        tree_.insertRangeUnique(std::ranges::begin(range),
    ...
    template <class... ArgsT>
    iterator
        try_emplace(const_iterator hint, const key_type& k, ArgsT&&... args) {
        return tree_
            .emplaceHintUnique(
                hint,
                std::piecewise_construct,
    ...
    iterator insert_or_assign(const_iterator hint, const key_type& k, _Vp&& v) {
        auto [r, inserted]
            = tree_.emplaceHintUnique(hint, k, std::forward<_Vp>(v));
        if (!inserted) {
    ...
    iterator insert_or_assign(const_iterator hint, key_type&& k, _Vp&& v) {
        auto [r, inserted]
            = tree_.emplaceHintUnique(hint, std::move(k), std::forward<_Vp>(v));
        if (!inserted) {
            r->value() = std::forward<_Vp>(v);
        }
        return r;
    ...
    iterator erase(const_iterator p) {
        return tree_.erase(p.i_);
    }
    iterator erase(iterator p) {
        return tree_.erase(p.i_);
    }
    size_type erase(const key_type& k) {
        return tree_.eraseUnique(k);
    }
    iterator erase(const_iterator f, const_iterator l) {
        return tree_.erase(f.i_, l.i_);
    }
    ...
    iterator insert(const_iterator hint, node_type&& nh) {
        MSTD_ASSERT_COMPATIBLE_ALLOCATOR(
            nh.empty() || nh.get_allocator() == get_allocator(),
            "node_type with incompatible allocator passed to map::insert()");
        return tree_.template nodeHandleInsertUnique<node_type>(hint.i_,
                                                                std::move(nh));
    }
    ...
    template <class CompareT2>
    void merge(
        multimap<key_type, mapped_type, CompareT2, allocator_type>& source) {
        MSTD_ASSERT_COMPATIBLE_ALLOCATOR(
            source.get_allocator() == get_allocator(),
            "merging container with incompatible allocator");
        tree_.nodeHandleMergeUnique(source.tree_);
    ...
    [[nodiscard]] size_type count(const TransparentKey& key) const {
        if constexpr (std::is_same_v<std::remove_cvref_t<TransparentKey>,
                                     key_type>) {
...
template <std::input_iterator                             IteratorT,
          std::sentinel_for<IteratorT>                    SentinelT,
          OrdersAtLeastWeakly<__iter_key_type<IteratorT>> CompareT
          = compare_three_way,
          Allocator AllocatorT
          = std::allocator<__iter_to_alloc_type<IteratorT>>>
map(IteratorT, SentinelT, CompareT = CompareT(), AllocatorT = AllocatorT())
    -> map<__iter_key_type<IteratorT>,
           __iter_mapped_type<IteratorT>,
           CompareT,
           AllocatorT>;

template <std::ranges::input_range                      RangeT,
          OrdersAtLeastWeakly<__range_key_type<RangeT>> CompareT
          = compare_three_way,
          Allocator AllocatorT = std::allocator<__range_to_alloc_type<RangeT>>>
map(RangeT&&, CompareT = CompareT(), AllocatorT = AllocatorT())
    -> map<__range_key_type<RangeT>,
...
template <class KeyT,
          ...
          OrdersAtLeastWeakly<KeyT> CompareT = compare_three_way,
          Allocator AllocatorT = std::allocator<MapValue<KeyT, ValueT>>>
map(std::initializer_list<MapValue<KeyT, ValueT>>,
    CompareT   = CompareT(),
    AllocatorT = AllocatorT())
    -> map<std::remove_const_t<KeyT>, ValueT, CompareT, AllocatorT>;

template <std::input_iterator          IteratorT,
          std::sentinel_for<IteratorT> SentinelT,
          Allocator                    AllocatorT>
map(IteratorT, SentinelT, AllocatorT) -> map<__iter_key_type<IteratorT>,
                                             __iter_mapped_type<IteratorT>,
                                             compare_three_way,
                                             AllocatorT>;

template <std::ranges::input_range RangeT, Allocator AllocatorT>
map(RangeT&&, AllocatorT) -> map<__range_key_type<RangeT>,
                                 __range_mapped_type<RangeT>,
                                 compare_three_way,
                                 AllocatorT>;
...
map(std::initializer_list<MapValue<KeyT, ValueT>>, AllocatorT)
    -> map<std::remove_const_t<KeyT>, ValueT, compare_three_way, AllocatorT>;
...
template <class KeyT, class ValueT, class CompareT, class AllocatorT>
auto operator<=>(const map<KeyT, ValueT, CompareT, AllocatorT>& x,
                 const map<KeyT, ValueT, CompareT, AllocatorT>& y) {
    using Map = map<KeyT, ValueT, CompareT, AllocatorT>;
...
template <class KeyT, class ValueT, class CompareT, class AllocatorT>
inline void swap(
    map<KeyT, ValueT, CompareT, AllocatorT>& x,
    map<KeyT, ValueT, CompareT, AllocatorT>& y) noexcept(noexcept(x.swap(y))) {
    x.swap(y);
}
...
inline typename map<KeyT, ValueT, CompareT, AllocatorT>::size_type
    erase_if(map<KeyT, ValueT, CompareT, AllocatorT>& container,
             PredicateT                               pred) {
...
class multimap {
  ...
  public:
    ...
    multimap() noexcept(std::is_nothrow_default_constructible_v<allocator_type>
                        && std::is_nothrow_default_constructible_v<key_compare>
                        && std::is_nothrow_copy_constructible_v<key_compare>)
    : tree_(key_compare()) {
    }
    ...
    template <_ContainerCompatibleRange<value_type> RangeT>
    multimap(RangeT&& range, const allocator_type& alloc)
    : multimap(std::forward<RangeT>(range), key_compare(), alloc) {
    }
    ...
    template <_ContainerCompatibleRange<value_type> RangeT>
    void insert_range(RangeT&& range) {
        tree_.insertRangeMulti(std::ranges::begin(range),
                               std::ranges::end(range));
    }
    ...
    iterator insert(const_iterator __hint, node_type&& nh) {
        MSTD_ASSERT_COMPATIBLE_ALLOCATOR(
            ...
            "node_type with incompatible allocator passed to "
            "multimap::insert()");
        return tree_.template nodeHandleInsertMulti<node_type>(__hint.i_,
                                                               std::move(nh));
    }
    [[nodiscard]] node_type extract(key_type const& __key) {
        return tree_.template nodeHandleExtract<node_type>(__key);
...
multimap(IteratorT, Sentinel, CompareT = CompareT(), AllocatorT = AllocatorT())
    -> multimap<__iter_key_type<IteratorT>,
                __iter_mapped_type<IteratorT>,
                CompareT,
                AllocatorT>;
...
multimap(RangeT&&, CompareT = CompareT(), AllocatorT = AllocatorT())
    -> multimap<__range_key_type<RangeT>,
                __range_mapped_type<RangeT>,
                CompareT,
                AllocatorT>;
...
template <class KeyT,
          ...
          OrdersAtLeastWeakly<KeyT> CompareT = compare_three_way,
          Allocator AllocatorT = std::allocator<MapValue<KeyT, ValueT>>>
multimap(std::initializer_list<MapValue<KeyT, ValueT>>,
         CompareT   = CompareT(),
         AllocatorT = AllocatorT())
...
template <std::input_iterator          IteratorT,
          std::sentinel_for<IteratorT> Sentinel,
          Allocator                    AllocatorT>
multimap(IteratorT, Sentinel, AllocatorT)
    -> multimap<__iter_key_type<IteratorT>,
                __iter_mapped_type<IteratorT>,
                compare_three_way,
                AllocatorT>;
...
template <std::ranges::input_range RangeT, Allocator AllocatorT>
multimap(RangeT&&, AllocatorT) -> multimap<__range_key_type<RangeT>,
                                           __range_mapped_type<RangeT>,
                                           compare_three_way,
                                           AllocatorT>;
...
multimap(std::initializer_list<MapValue<KeyT, ValueT>>, AllocatorT)
    -> multimap<std::remove_const_t<KeyT>,
                ValueT,
                compare_three_way,
                AllocatorT>;
...
template <class KeyT, class ValueT, class CompareT, class AllocatorT>
auto operator<=>(const multimap<KeyT, ValueT, CompareT, AllocatorT>& x,
                 const multimap<KeyT, ValueT, CompareT, AllocatorT>& y) {
    using Map = multimap<KeyT, ValueT, CompareT, AllocatorT>;
...
template <class KeyT, class ValueT, class CompareT, class AllocatorT>
inline void swap(multimap<KeyT, ValueT, CompareT, AllocatorT>& x,
                 multimap<KeyT, ValueT, CompareT, AllocatorT>&
...
inline typename multimap<KeyT, ValueT, CompareT, AllocatorT>::size_type
    erase_if(multimap<KeyT, ValueT, CompareT, AllocatorT>& container,
             PredicateT                                    pred) {
    return mstd::erase_if_container(container, pred);
}
...
using map = mstd::map<KeyT,
                      ValueT,
                      _CompareT,
                      std::pmr::polymorphic_allocator<MapValue<KeyT, ValueT>>>;
template <class KeyT, class ValueT, class _CompareT = compare_three_way>
using multimap
    = mstd::multimap<KeyT,
                     ValueT,
